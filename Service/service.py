...
def video_capture():
    ...
    while True:
        ...
        if results.pose_landmarks:
            ...
            # lshoulder = [results.pose_landmarks.landmark[mpPose.PoseLandmark.LEFT_SHOULDER].x, results.pose_landmarks.landmark[mpPose.PoseLandmark.LEFT_SHOULDER].y]
            ...
            if pushup_check(lankle, rankle, lknee, rknee, lhip, rhip):
                ...
                if min([0 if x!="correct" else 1 for x in state_stack[max(len(state_stack)-5,0):]])==1:
                    nr_of_pushups+=1
                prev_state="correct"
            else:
            ...
            state_stack.append(prev_state)
            while len(state_stack)>10:
                state_stack.pop(0)
            print([0 if x!="correct" else 1 for x in state_stack[max(len(state_stack)-5,0):]])
